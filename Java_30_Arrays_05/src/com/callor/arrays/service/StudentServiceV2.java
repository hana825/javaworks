 ...
 * 상속을 받으면 ServiceV1에 선언된 변수, method를 그대로 물려받는다. (단, 생성자 제외)
 * 
...
public class StudentServiceV2 extends StudentServiceV1 {
	...
	/*
	 * Overrride(재정의) : ServiceV1에 선언된 method 중에서 inputStudent 메서드를 다시 정의하겠다.
	 */
	@Override
	public int inputStudent() {
		...
		for(index = 0; index < strStudents.length; index++) {
			...
			strStudents[index] = strName;	// strStudents[]에 console에 입력한 값 저장
		} // for()
		/*
		 * for()가 시작되기전에 선언된 index는 for(){}가 종료되어도 값을 유지하고 있다.
