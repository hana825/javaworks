...
public class StringEx1 {
	...
	public static void main(String[] args) {
		...
		// for() 반복문이 문자열의 길이만큼 반복될 때 계속해서 문자열의 길이를 계산하는 코드가 실행된다.
		for(int i = 0; i < str1.length(); i++) {
			System.out.println(str1.substring(i, i + 1));
		}
		System.out.println();
		
		// for() 반복문이 실행되기 전에 문자열의 개수를 계산하여 return하는 length() method를 호출하고 결과는 변수에 담는다.
		// 이제는 문자열의 길이를 계산하는 코드는 한 번만 실행되고 문자열의 길이만큼 for() 반복문을 수행할 수 있게 되었다.
