...
//			 random() * 50을 연산하면 0 ~ 49까지 범위의 임의 수가 생성된다.
//			 여기에 +51을 하면 0일 때는 51, 49일 때는 100의 결과가 생성된다.
//			 그래서 51 ~ 100까지의 범위에서 임의 수를 생성하는 코드
			intKor[i] = (int)(Math.random() * 50) + 51;
		}
	} // makeScore()
	public void printScore() {
//		Line 클래스에 선언된 dLine 속성(변수)는 static이기 때문에 Line 클래스를 line 인스턴스로 만들고 line.dLine처럼 접근할 필요가 없다.
//		Line line = new Line();
//		System.out.println(line.dLine);
		
//		 dLine 속성(변수)은 static으로 선언되어 있기 때문에 Line 클래스를 인스턴스로 생성하지 않고도 접근할 수 있다.
