...
public class StudentVO {
	...
	/*
	 * VO 클래스에 기본생성자와 모든 인스턴스변수(클래스영역의 변수)를 매개변수로 받아 초기화하는 임의생성자를 만든다.
	 * 
	 * JDK를 사용하여 프로젝트를 수행할 때 다양한 도구들을 연결할 필요가 있는데 도구들마다 VO 객체에 접근하는 방법이 조금씩 다르다.
	 * 어떤 도구는 getter, setter를 통하여 접근하기도 하고
	 * 어떤 도구는 필드생성자를 통하여 접근하기도 한다.
	 * 
	 * 최근에는 getter, setter를 만들지 말자라는 의견이 있지만 다양한 JDK도구를 사용할 때는 필요하기도 하므로
	 * 표준 Java 코딩의 VO클래스 작성 패턴에서는 모든 변수를 private으로 선언하고 getter, setter를 정의해준다. 
	 */
	// Constructor
	public StudentVO() {
	...
	// VO 객체에 데이터를 저장한 후 저장된 데이터가 잘 담겼는지 확인하는 용도로 사용하기 위하여 toString() 메서드를 재정의한다.
	@Override
