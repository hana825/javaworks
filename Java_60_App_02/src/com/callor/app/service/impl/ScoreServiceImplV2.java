...
public class ScoreServiceImplV2 extends ScoreServiceImplV1 {
		...
		while (true) {
			...
			for(index = 0; index < strSubject.length; index ++) {
				...
				if(intScore[index] == null) {
					/* 만약 index 값이 0이라면 "국어점수" 입력화면이 나타나다.
					 * 점수를 숫자로 입력하지 않고 Enter만 누르거나 다른 문자열이 섞인 경우라면
					 * "국어점수는 정수만 입력"메시지를 보여주고 null을 return할 것이다.
					 * 그러면 index값을 1 감소시켜 -1로 만든후 다시 for()처음으로 점프한다.
					 * for(index = -1; ...){}가 다시 실행될 것이다.
					 * for( ;; index++)에 의해서 index는 다시 0이된다. 그러면 국어점수를 입력하는 화면이 다시 나타날 것이다.
					 */
					index --;
					continue;
				}
	...
	// 정수 점수를 정확히 입력했으면 점수를 return하고 만약 exception이 발생하면 null을 return
	// 종료(QUIT) 입력하면 -1을 return
