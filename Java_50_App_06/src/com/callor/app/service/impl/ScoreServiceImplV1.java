...
public class ScoreServiceImplV1 implements ScoreService {
	...
	/*
	 * Service 클래스에서 필요한 데이터를 클래스 영역에 선언할 때는 final 키워드를 부착한다.
	 * final 키워드를 부착하면 혹시 모를 데이터 손상을 방지하고 데이터가 저장된 상태에서 임의의 또 다시 초기화 하는 것을 방지하기
	 * 초기화 하지 않으면 오류가 나서 NullPointerException도 막을 수 있다.
	 */
	
	// 리스트 객체를 final로 선언하고
	private final List<ScoreVO> scList;
	public ScoreServiceImplV1() {
		// 생성자에서 리스트 객체를 초기화하여 NullpointerException을 사전에 예방한다.
