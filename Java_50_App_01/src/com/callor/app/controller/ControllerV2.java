...
public class ControllerV2 {
	...
	public static void main(String[] args) {
		...
		// ServiceV2에는 기본 생성자가 없기 때문에 기본생성자를 호출하여 인스턴스를 생성할수 없다.
		// 그 이유는 ServiceV2에서는 내부에서 사용하는 intList를 초기화하는 코드가 없고, 
		// ServiceV2를 호출하는 외부(Controller)에서 초기화된 list를 매개변수로 주입해서 사용해야 하기 때문에.
		// 만약 기본 생성자를 호출하여 객체를 만들게 되면 intList에서 Nullpointer Exception이 발생할 것이다.
		// 그러한 이유로 기본생성자를 아예 호출하지 못하도록 만든다. 
		// 기본생성자에서 intList를 초기화하는 코드를 추가할 수 있지만, ServiceV2 클래스의 생성자가 복잡해질 수있다. 
//		ServiceV2 sV2 = new ServiceV2();
		
		List<Integer> scoreList = new ArrayList<>();
		ServiceV2 sV2 = new ServiceV2(scoreList);
		
		// List interface를 상속받은 구현클래스들 ...
		...
		ServiceV2 sv6 = new ServiceV2(list3);
