...
public class ObjectEx2 {
	...
	public static void main(String[] args) {
		...
		/* 배열 객체를 makeScore() 메서드에 매개변수로 전달
		 * 
		 * 매개변수로 scVOs를 makeScore() 메서드에 전달하고 makeScore() 메서드에서 배열의 요소를 각각 변경(setting)하면
		 * main() 메서드에서 선언된 scVOs 배열의 값들이 변경된다.
		 * 이러한 성질을 갖는 변수를 "참조형변수"라고 한다.
		 * 
		 * 참조형변수의 성질을 잘 이용하면 method가 실행된 후 결과를 return하지 않아도 된다.
		 */
		makeScore(scVOs);
		printScore(scVOs);
	} // end main
