...
public class StudentDao {
	 ...
	 * 2. Exception을 나를 호출할 곳으로 던지기
	 * 
	 *  Service에서 다음과 같이 selectAll()을 호출할 것이다.
	 * 		List<StudentVO> stList = dao.selectAll();
	 * 	dao.selectAll()에서 exception이 발생하면 dao.selectAll()에서 exception을 직접 처리(핸들링)하지 않고
	 *  Service로 exception 던지기(전달)하여 exception을 모아서 처리한다.
	 */
